...
const getMedidas = data => {
  ...
  aux.forEach(p_integralizado => {
    const filtro = data.filter(
      e => e.periodos_integralizados === p_integralizado
    );
    let soma = 0;
    filtro.map(e => {
      soma += e.porcentagem_concluida;
      return null;
    });
...
const getDataScatter = data => {
  ...
  if (data) {
    data.map((entry, index) => {
      const [medida] = medidas.filter(
        e => e.x === entry.periodos_integralizados
      );

      if (entry.porcentagem_concluida >= 100) {
        purple.push(entry);
      } else if (
        entry.porcentagem_concluida < medida.y &&
        entry.porcentagem_concluida < medida.ideal
      ) {
        red.push(entry);
      } else if (
        entry.porcentagem_concluida >= medida.y &&
