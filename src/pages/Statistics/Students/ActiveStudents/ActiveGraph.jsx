    ...
    if (active) {
      return (
        <div className='custom-tooltip'>
          <p>
            Períodos Integralizados:{" "}
            {payload[0].payload.periodos_integralizados}
          </p>
          <p>
            Créditos Integralizados: {payload[0].payload.porcentagem_concluida}%
          </p>
        </div>
      );
    }
        ...
        <React.Fragment>
          <div className='graph-main'>
            ...
            >
              ...
              <Scatter
                data={green}
                onClick={e => handleScatter(e, green)}
                fillOpacity={0.5}
                fill={"green"}
              ...
              <Scatter
                data={blue}
                onClick={e => handleScatter(e, blue)}
                fillOpacity={0.5}
                fill={"blue"}
                name={"Ideal"}
              ></Scatter>
              <Scatter
                data={purple}
                onClick={e => handleScatter(e, purple)}
                fillOpacity={0.5}
                fill={"purple"}
          ...
          <div className='ativos-texto'>
            <p>
              Existem <strong>{props.data.length}</strong> discentes ativos com
              ingresso entre <strong>{min}</strong> e <strong>{max}</strong>.{" "}
              <strong>{blue.length}</strong> ({getPercentagem(props, blue)}%)
              dos discentes ativos estão com a execução curricular no patamar
              ideal, <strong>{green.length}</strong> (
              {getPercentagem(props, green)}%) estão dentro do esperado,{" "}
              <strong>{purple.length}</strong> ({getPercentagem(props, purple)}
              %) estão acima do esperado, enquanto que{" "}
              <strong>{red.length}</strong> ({getPercentagem(props, red)}%)
              estão com a execução curricular abaixo do esperado.
              <strong>{periodDown}</strong> é o semestre com mais discentes com
              execução curricular abaixo do esperado ({periodDownValue}).
            </p>
          </div>
          <Modal
            backdrop={true}
